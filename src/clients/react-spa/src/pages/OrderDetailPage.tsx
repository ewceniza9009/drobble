// ---- File: src/clients/react-spa/src/pages/OrderDetailPage.tsx ----
...
const StatusBadge = ({ status }: { status: string }) => {
    const statusConfig: { [key: string]: { class: string; icon: JSX.Element } } = {
        Pending: { class: 'bg-yellow-100 text-yellow-800 ring-1 ring-inset ring-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-300 dark:ring-yellow-500/30', icon: <FaCalendar className="mr-1" /> },
        Paid: { class: 'bg-green-100 text-green-800 ring-1 ring-inset ring-blue-200 dark:bg-green-900/30 dark:text-green-300 dark:ring-green-500/30', icon: <FaReceipt className="mr-1" /> },
        Shipped: { class: 'bg-indigo-100 text-indigo-800 ring-1 ring-inset ring-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-300 dark:ring-indigo-500/30', icon: <FaTruck className="mr-1" /> },
        Delivered: { class: 'bg-green-100 text-green-800 ring-1 ring-inset ring-green-200 dark:bg-green-900/30 dark:text-green-300 dark:ring-green-500/30', icon: <FaBox className="mr-1" /> },
        Cancelled: { class: 'bg-red-100 text-red-800 ring-1 ring-inset ring-red-200 dark:bg-red-900/30 dark:text-red-300 dark:ring-red-500/30', icon: <FaShoppingBag className="mr-1" /> },
        Refunded: { class: 'bg-slate-100 text-slate-800 ring-1 ring-inset ring-slate-200 dark:bg-slate-700 dark:text-slate-300 dark:ring-slate-600', icon: <FaReceipt className="mr-1" /> },
    };
    const config = statusConfig[status] || { class: 'bg-slate-100 text-slate-800', icon: <FaShoppingBag className="mr-1" /> };
    return ( <span className={`px-3 py-1.5 text-sm font-medium rounded-full flex items-center w-fit ${config.class}`}>{config.icon} {status}</span> );
};
    ...
    const handleCancelOrder = async () => {
        ...
        if (window.confirm("Are you sure you want to cancel this order? This action cannot be undone.")) {
            ...
            toast.promise(promise, {
                ...
                success: () => { navigate('/profile'); return 'Order cancelled successfully.'; },
                ...
                <div className="lg:col-span-2">
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md border border-gray-200 dark:border-slate-700">
                        <h2 className="text-xl font-semibold mb-6 text-gray-800 dark:text-slate-100 flex items-center">
                            ...
                            Items Ordered ({enrichedItems.length})
                        </h2>
                    ...
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 sticky top-8">
                        ...
                        <div className="space-y-3 mb-4">
                            <div className="flex justify-between text-gray-600 dark:text-slate-400">
                                ...
                                <span className="text-gray-800 dark:text-slate-200">{formatCurrency(subtotal)}</span>
                            </div>
                            <div className="flex justify-between text-gray-600 dark:text-slate-400">
                                <span>Shipping</span>
                                <span className="text-gray-800 dark:text-slate-200">{formatCurrency(order.shippingCost)}</span>
                            </div>
                            <div className="flex justify-between text-gray-600 dark:text-slate-400">
                                <span>Tax (8%)</span>
                             ...
                             {order.discountAmount > 0 && (
                                <div className="flex justify-between text-green-600 dark:text-green-400">
                                    <span>Discount ({order.appliedPromoCode})</span>
                        ...
                        <div className="mt-6 pt-6 border-t border-gray-200 dark:border-slate-700">
                            ...
                            <div className="space-y-2 text-sm">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span className="text-gray-800 dark:text-slate-200">{order.paymentMethod}</span>
                                </div>
                            </div>
                        </div>
                    </div>
